 ...
 *
 * 直接照搬 web/Toonflow-app/src/utils/stripThink.ts 的实现，逻辑保持一致。
 */
      ...
      while (i < chunk.length) {
        if (insideThink) {
          ...
          if (closeIdx !== -1) {
            ...
            // 整个剩余 chunk 都在 think 内，除非末尾是不完整的 "</think>" 片段
            const potentialCloseStart = findPartialTag(chunk, i, '</think>');
            if (potentialCloseStart !== -1) {
              buffer = chunk.slice(potentialCloseStart);
            }
            break;
          }
        } else {
          ...
          if (openIdx !== -1) {
            ...
            // 缓冲末尾可能是 "<" 开头的不完整标签片段
            const potentialStart = findPartialTag(chunk, i, '<think>');
            if (potentialStart !== -1) {
 ...
 */
function findPartialTag(chunk: string, startIdx: number, tag: string): number {
